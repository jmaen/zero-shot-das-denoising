...
\begin{figure}[t]
    ...
    \caption{
        ...
        Image quality is evaluated on 20 random images from the CelebA (left) and CBSD68 (right) datasets, with Gaussian noise added at specified levels.
...
\begin{equation}
    \theta^*,z^* = \argmin_{\theta, z} \norm{\E[\epsilon]{f_\theta(z + \epsilon)} - y}_2^2 + \lambda\norm{\E[\epsilon]{f_\theta(z + \epsilon)} - z}_2^2,
\end{equation}
where $y$ refers to the noisy image, $z$ is a random tensor, $\epsilon$ is Gaussian noise sampled as $\epsilon \sim \normal$ and $\lambda$ is a hyperparameter balancing the two terms.
...
The expectation over the noise distribution helps to stabilize the learned mapping and, in practice, is approximated by averaging the outputs for multiple noise samples.
...
Due to the regularization term, both the input and output should provide a reasonable estimate of the clean image.
...
To explicitly reinforce this behavior, we propose using a scheduler, inspired by DDIP, to gradually increase $\lambda$ throughout training.
...
% TODO use y as input to simplify initial signal extraction (increase \epsilon variance to prevent learning identity)
% TODO figures - process / schedules?
