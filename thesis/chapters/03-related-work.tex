...
Zhang~et al.\ proposed the denoising convolutional neural network (DnCNN)~\cite{DnCNN} which improves denoising performance by making use of residual learning, i.e.\ instead of directly predicting the clean image, it is trained to predict the noise in the noisy image.
...
To address this issue, Lehtinen~et al.\ proposed Noise2Noise (N2N)~\cite{N2N}, which does not require any clean data.
...
\begin{equation}\label{eq:N2N}
    \argmin_\theta \sum_{i=1}^{n} ||f_\theta(y_1^i) - y_2^i||_2^2.
\end{equation}
...
\begin{wrapfigure}{r}{0.48\textwidth}
    ...
    \begin{tikzpicture}[tdplot_main_coords, scale=0.9]
        ...
        \fill[color=Maroon!50] (-1+3,2,3) -- (-1+4,2,3) -- (-1+4,2,4) -- (-1+3,2,4) -- cycle;
        \foreach \i in {1,2,3,4} {
            \draw (-1,2,\i) -- (-1+5,2,\i);
        ...
        \fill[color=Maroon!50] (-2+2,4,2) -- (-2+3,4,2) -- (-2+3,4,3) -- (-2+2,4,3) -- cycle;
        \foreach \i in {1,2,3,4} {
            \draw (-2,4,\i) -- (-2+5,4,\i);
...
Krull~et al.\ first introduced this concept in their Noise2Void (N2V) paper~\cite{N2V}.
...
\begin{equation}
    \argmin_\theta \sum_{i=1}^{n} ||f_\theta(y^i_{J^c})_J - y^i_J||_2^2.
\end{equation}
