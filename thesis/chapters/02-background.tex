...
\begin{equation}
    (\mathbf{X} \ast \mathbf{K})_{i,j} = \sum_{k=1}^{m}\sum_{l=1}^{n} X_{i+m,j+n} \cdot K_{k,l}.
\end{equation}
...
For an output~$x \in \R^{W \times H \times C}$ of a convolutional layer, where $W$, $H$ and $C$ denote width, height and channel dimension, respectively, the weight of a feature channel $i$ is given by
\begin{equation}
